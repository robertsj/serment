//----------------------------------*-C++-*-----------------------------------//
/**
 *  @file  test_AbsorptionOperator.cc
 *  @brief Test of AbsorptionOperator class.
...
int test_AbsorptionOperator(int argc, char *argv[])
{
  ...
  {
    // AbsorptionOperator
    AbsorptionOperator A(nodes, indexer, server);
    ...
    double A_times_J = A.dot(x);

    double ref = 0;
    for (int n = 0; n < nodes->number_global_nodes(); n++)
      ref += 4 * n * 1000000.0 + (0 + 1 + 2 + 3) * 100000.0 + 1.2;

    TEST(soft_equiv(A_times_J, ref));
  }
...
//----------------------------------------------------------------------------//
//              end of test_AbsorptionOperator.cc
//----------------------------------------------------------------------------//
